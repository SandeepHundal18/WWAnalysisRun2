...
//if (jentry2==60000) continue ;
	...
	//    MCpu_down->Fill(info->nPUmeanm);
	  if (jentry2%10000 == 0)std::cout << "\t File no. " << i << "; Neg Event Count; read entry: " << jentry2 <<"/"<<TotalNumberOfEvents<<std:: endl;
...
// cout <<"number of lhe weights="<<gen->lheweight.size()<<endl;
for (unsigned int i = 0; i<gen->lheweight.size();i++)
{
//const baconhep::TGenEventInfo* gen1 = (baconhep::TGenEventInfo*) ((*gen)[i]);
//WWTree->LHEWeight[i] = genBr->lheweight;
//cout<<"test for event ="<<jentry<<" " <<gen->lheweight.at(i)<<endl;
WWTree->LHEWeight[i] =gen->lheweight.at(i);
}
	...
	{ //loop13 begins
	  ...
	  WWTree->nu2_eta_gen	  = (v_genNeutrino[1]).Eta();
         ...
         // WWTree->lep1_pdgid     = v_genLep[0].pdgId();
        //  WWTree->lep2_pdgid     = v_genLep[1].pdgId();

          WWTree->dilep_m_gen    = (v_genLep[0]+v_genLep[1]).M();
//cout <<"    "<< WWTree->dilep_m_gen<<endl;    
...
//cout<<"check3--all good"<<endl;
	...
	if ( WWTree->lep1_pt_gen > 25 && WWTree->lep2_pt_gen >20 && WWTree->dilep_m_gen >20   && abs(WWTree->AK4_jj_DeltaEta_gen)>2.5 && WWTree->AK4_1_pt_gen >30 && WWTree->AK4_2_pt_gen>30 && WWTree->AK4_jj_mass_gen > 500  && max(WWTree->Zeppen1,WWTree->Zeppen2)<0.75  ) 
// && WWTree->WW_mass_gen<4110
         {
	        GenPassCut = 1;
	 }
if (!( WWTree->lep1_pt_gen > 25 && WWTree->lep2_pt_gen >20 && WWTree->dilep_m_gen >20   && abs(WWTree->AK4_jj_DeltaEta_gen)>2.5 && WWTree->AK4_1_pt_gen >30 && WWTree->AK4_2_pt_gen>30 && WWTree->AK4_jj_mass_gen > 500  && max(WWTree->Zeppen1,WWTree->Zeppen2)<0.75  ) ) continue;

//   for (int i = 0; i<gen->GetEntries();i++)     // Note that i is starting from 446.
//	{
//	const baconhep::GenEvtInfo *genlhe = (baconhep::GenEvtInfo*)((*gen)[i]);
		//WWTree->LHEid[i] = lhe->id;
//		WWTree->LHEWeight[i] = gen->lheweight[i];
//}
    }//loop14 ends
